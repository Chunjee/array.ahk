...
| :------------- | :--------------------------------------------------- | :----------- |
|  value*        | value/object | Optional. (Default `[]`) Arrays and/or values to concatenate into a new array. If all valueN parameters are omitted, concat returns a shallow copy of the existing array on which it is called. |


#### Returns
(array): A new Array instance. Containing the values of the joined arrays.
...
| Argument       | Type                       | Description  |
| :------------- | :------------------------- | :----------- |
|  value         | value | Required. The value to fill the array with. |
|  start         | number | Optional. (Default `1`) The index to start filling the array. If negative, it is treated as array.Count() + start |
|  end           | number | Optional. (Default `array.Count()`) The value to fill the array with. If negative, it is treated as `array.Count() + end` |
...
#### Returns
(value): The value of the first element in the array that satisfies the provided testing function. Otherwise, `""` if none found.
...
#### Returns
(value): The index or key value of the first element in the array that satisfies the provided testing function. Otherwise, `-1` if not found.
...
| Argument       | Type         | Description  |
| :------------- | :----------- | :----------- |
|  searchElement | value        | Required. The value to search for. |
|  fromIndex     | number       | Optional. (Default `1`) The index to start the search at. If the provided index value is a negative number, it is taken as the offset from the end of the array. |


#### Returns
(number): The index or key value of the first element in the array that matches the searchElement. Otherwise, `-1` if not found.
...
#### Returns
(number): The last index of the element in the array
...
Returns a shallow copy of a portion of an array into a new array object selected from start to end (end not included) where start and end represent the index of items in that array. The original array is not be modified.
...
#### Returns
(value): A new array containing the extracted elements.
...
| Argument       | Type         | Description  |
| :------------- | :----------- | :----------- |
|  function(element, index, array)      | function     | Required. A function to execute for each element in the array. It should return a truthy value to indicate the element passes the test, and a falsy value otherwise. |
...
|  values*        | value/object | Optional. (Default `""`) The values to add to the array. If you do not specify any elements, it will only remove from the array. |
...
#### Returns
(string): A string representing all elements of the array.
...
| Argument       | Type         | Description  |
| :------------- | :----------- | :----------- |
|  element*      | value/object | Required. The elements to add to the front of the array. |
